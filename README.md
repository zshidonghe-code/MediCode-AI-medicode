...
**编码准确性（小样本验证）**：目前仅有 **4 例金标准验证性测试**（`tests/benchmark_accuracy.py`，无 LLM 本地索引模式），4/4 全部通过；样本量太小，**不构成可信准确率基准**，建立经医学专家审核的标准病例集是当前最高优先级事项。`920` 条诊断和 `611` 条手术为编码字典规模，不是测试集规模。

**质控缺陷发现能力（400 份注入式基准）**：采用注入已知缺陷法——在 400 份合成病历（340 份注入已知缺陷 + 60 份干净对照）上测试规则层质控引擎：**缺陷召回率 340/340（100%），17 条规则全部可触发；干净病历误报率 0%**（编码引擎手术候选过宽问题已修复，详见 docs/QC_BENCHMARK_REPORT.md）。注入式方法不需要专家标注金标准，可与开发同步回归。

测试方法、执行结果和限制详见 `docs/QC_BENCHMARK_REPORT.md` 与 `docs/BENCHMARK_REPORT.md`。系统定位为提交前辅助审核工具，不能替代编码员进行无人审核的最终编码。
...
2. 演示账号（密码统一 `123456`）：
   ...
   - 编码员：`coder` / `123456`
   - 医生：`doctor` / `123456`
3. 开发环境首次启动自动填充演示数据（500 条病历；由 `AUTO_SEED_DEMO_DATA=true` 控制）
