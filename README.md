...
**质控缺陷发现能力（400 份注入式基准）**：采用注入已知缺陷法——在 400 份合成病历（340 份注入已知缺陷 + 60 份干净对照）上测试规则层质控引擎：**缺陷召回率 340/340（100%），17 条规则全部可触发；干净病历误报率 21.7%**，根因已定位为编码引擎手术候选过宽（详见改进方向）。注入式方法不需要专家标注金标准，可与开发同步回归。
