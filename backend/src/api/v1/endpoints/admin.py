    ...
    async with async_session() as db:
        ...
        if not body.confirm:
            return {
                ...
                + "参考数据（icd_codes/drg_groups/qc_rules）不受影响。发送 confirm=true 确认执行。",
                "counts": counts,
            }
...
async def export_coding_results(
    ...
    async with async_session() as db:
        r = await db.execute(
            select(CodingResult, MedicalRecord.record_type).join(
                MedicalRecord, CodingResult.record_id == MedicalRecord.id, isouter=True
            )
        )
        rows = []
        for cr, rec_type in r.all():
            ...
            def _extract_codes(items: object) -> str:
                ...
                for item in items:
                    if isinstance(item, dict):
                        result.append(str(item.get("code", str(item))))
                    else:
                        result.append(str(item))
                return "|".join(result)

            def _extract_primary(val: object) -> str:
...
async def export_patient_summaries(
    ...
    async with async_session() as db:
        ...
        all_records = (
            await db.execute(
                select(
                    MedicalRecord.patient_id, MedicalRecord.record_type, MedicalRecord.department
                )
            )
        ).all()
            ...
            rows.append(
                {
                    ...
                    "departments": "|".join(
                        sorted({r.department for r in records if r.department})
                    ),
