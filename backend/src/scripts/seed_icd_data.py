...
from src.models.database import async_session, init_db, engine, Base
...
ICD10_CODES: list[dict] = _load_json("icd_diagnoses.json")
ICD9_PROCEDURES: list[dict] = _load_json("icd_procedures.json")
...
async def seed():
    ...
    async with async_session() as session:
        ...
        for item in ICD10_CODES:
            ...
            existing = await session.execute(
                select(ICDCode).where(ICDCode.code == code, ICDCode.version == ICDVersion.ICD10_CLINICAL)
            )
    ...
    print(f"Seeded {diag_count} ICD-10 diagnosis codes and {proc_count} ICD-9-CM-3 procedure codes.")
