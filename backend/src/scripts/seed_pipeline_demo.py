...
QC_ISSUE_POOL = [
    ...
    },
    {
        "rule_id": "QC-104",
        "severity": "major",
        "desc": "住院天数异常偏长，请确认是否有特殊原因",
        "suggestion": "住院天数与诊断/手术复杂度不匹配",
    },
    {
        "rule_id": "QC-201",
        "severity": "major",
        "desc": "诊断编码中的关键临床术语在病历文本中未找到",
    ...
    },
    {
        ...
        "severity": "critical",
        "desc": "主要诊断为症状而非病因",
        "suggestion": "应选择病因为主要诊断",
    },
    {
        "rule_id": "QC-301",
        "severity": "minor",
        "desc": "诊断名称口语化，建议使用规范术语",
        "suggestion": "请使用ICD规范诊断名称",
    },
    {
        "rule_id": "QC-302",
        "severity": "minor",
        "desc": "缺少手术日期记录",
        "suggestion": "手术记录必须包含精确手术日期",
    },
    {
        "rule_id": "QC-005",
        "severity": "major",
        "desc": "入院记录超24小时完成",
        "suggestion": "入院记录应在患者入院24小时内完成",
    },
    {
        "rule_id": "QC-202",
        "severity": "major",
        "desc": "漏编次要诊断",
        "suggestion": "病历提及的慢性病应在次要诊断中完整编码",
    },
]
...
async def seed_pipeline_demo():
    async with async_session() as db:
        ...
        for code, name, weight, surgical in drg_data:
            existing = (
                await db.execute(select(DRGGroup).where(DRGGroup.code == code))
            ).scalar_one_or_none()
            if not existing:
        ...
        for patient_idx, tmpl in enumerate(expanded_pool, start=1):
            (
                dept,
                gender_pref,
                age_range,
            ...
            record = MedicalRecord(
                ...
                department=dept,
                doctor_hash=hashlib.sha256(
                    f"demo_doctor_{random.randint(1, 20)}".encode()
                ).hexdigest()[:64],
        ...
        print("Pipeline demo seed complete!")
