...
tech_data = [
    ['规则模式主诊断匹配2/4', '码医内部工程回归\n4例合成病历', '3科室(心内/呼吸/骨科)\nLLM未启用\n规则引擎模式', '4例结果和限制见 docs/BENCHMARK_REPORT.md；不能据此宣称生产级准确率。'],
    ['诊断F1得分84.5%', '同上', 'Top-3推荐下的综合得分\n精确率+召回率加权', 'F1比Top-1更能反映综合表现。\n84.5%意味着AI推荐的Top-3编码中，\n绝大多数情况下包含了正确答案。'],
    ['手术编码验证', '4例规则模式工程回归', '611条当前手术编码\n覆盖范围仍需扩大', '当前样本不足以计算可推广的F1；后续需在专家复核病例集上评估。'],
    ['47条质控规则，6大维度', '码医质控引擎设计文档', '完整性12+逻辑一致性8+编码一致性7\n+时效性5+规范表达9+语义质量6', '规则来自国家病案质控标准+临床指南。\n每条规则都有明确的触发条件和缺陷等级。'],
    ['ICD-10当前为920条', '当前数据规模，非完整编码集', '覆盖率尚未验证\n低频编码覆盖仍需评估', '当前数据用于工程验证，不能推导出出院病例覆盖率；编码库按授权数据和评审结果持续扩展。'],
]
