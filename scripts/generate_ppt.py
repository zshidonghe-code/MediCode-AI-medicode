...
add_content_slide('码医：AI驱动的一体化解决方案', [
    ('NLP智能编码 ', '→ ICD-10诊断(920条) + ICD-9-CM-3手术(611条)，4例规则模式验证：主诊断2/4、手术2/4'),
    ('DRG自动分组 ', '→ CHS-DRG 1.2方案，26 MDC，权重+费率+预估支付'),
    ('病历质控 ', '→ 内置质控规则 + 可选LLM语义检查，最终由人工审核'),
    ('数据驾驶舱 ', '→ CMI分析、收入趋势、质控排行，管理者实时决策'),
    ('智能流水线 ', '→ 一份病历进去，编码+DRG+质控+费用全流程输出'),
    '---',
    ('审核流程 = ', '编码 + DRG分组 + 质控 + 拒付风险评估，最终由人工确认'),
])
...
add_content_slide('Pipeline：四步完成全流程分析', [
    ...
    '---',
    ('演示模式：', '快速演示（比赛首选，规则引擎1秒出结果，离线安全）+ 完整演示（LLM增强+打字机动画）'),
])
...
add_content_slide('团队：用AI工具高效执行', [
    ('郑诗东和 ', '— 项目负责人，上海对外经贸大学。方向决策、商业策划、路演答辩、医院合作洽谈'),
    ('', '— 自学ICD编码规则和CHS-DRG分组方案，完成编码员深度访谈'),
    ('技术开发 ', '— 利用AI辅助开发工具（Claude），单人2个月从零搭建完整全栈系统'),
    ('', '— 8个前端页面+7组API+8张数据库表+17个单元测试，传统团队需3-6个月'),
    ('医学顾问 ', '— 已与医学院和医院编码科取得联系，正在进行合作洽谈'),
    '---',
    ('不是PPT创业——规则模式全链路可运行，当前验证记录和限制已归档，现场可演示核心流程。', ''),
])
