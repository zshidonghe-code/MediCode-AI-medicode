...
add_para('系统已完整开发完成，可现场演示任何功能。核心数据：870条ICD-10诊断编码 + 558条ICD-9-CM-3手术编码（基于CHS-DRG 2.0方案）。50份模拟病历基准测试：诊断Top-1准确率98%，F1得分83.1%。双引擎架构（规则引擎保底+大模型增强），支持完全离线运行。Docker一键私有化部署，病历数据不出医院内网。')
...
add_table_simple(['指标', '数据'], [
    ...
    ['后端测试', '17个单元测试全部通过'],
    ['ICD诊断编码库', '870条（覆盖14个系统，基于CHS-DRG 2.0方案）'],
    ['ICD手术编码库', '558条（覆盖9个专科）'],
...
add_bullet('"人+AI"新协作模式：AI Agent负责技术研发（有完整代码库为证），人类负责人聚焦商业和路演。效率远超传统纯人类小团队——不到2个月完成10人团队3-6个月的开发量。')
add_bullet('不是PPT创业：已有完整可运行的全栈系统，评审可现场验证任何功能。')
add_bullet('赛道精准匹配：医疗AI + 医保改革 = 评审最关注的热点方向。')

doc.add_heading('5.3 顾问团队规划', level=2)

add_table_simple(['角色', '理想背景', '预期贡献', '招募计划'], [
    ['医学顾问', '三甲医院编码科/病案室主任\n或医学院卫生管理教授',
     '编码规则审核、质控规则验证\n编码准确率测试',
     '赛后1个月内通过大赛评委/\n校友/行业网络确定'],
    ['商业顾问', '医疗SaaS创业者\n或医疗信息化资深从业者',
