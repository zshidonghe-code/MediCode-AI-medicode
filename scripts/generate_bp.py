...
add_para('系统已完整开发完成，可现场演示任何功能。核心数据：920条ICD-10诊断编码 + 571条ICD-9-CM-3手术编码（基于CHS-DRG 2.0方案，编码库处于持续扩充中，计划覆盖方案中所有高频诊断编码）。203份模拟病历基准测试：诊断Top-1准确率94.1%，F1得分84.5%。双引擎架构（规则引擎保底+大模型增强），支持完全离线运行。Docker一键私有化部署，病历数据不出医院内网。')
...
add_bullet('ICD数据源统一JSON管理：920条诊断+571条手术编码，所有消费者从同一数据源加载')
...
add_para('我们使用203份模拟临床病历进行了系统性的编码准确率基准测试，覆盖8个科室（心内科/呼吸科/消化科/内分泌科/神经内科/骨科/普外科/妇产科）。', bold=True)

add_table_simple(['指标', '数值', '说明'], [
    ['诊断 Top-1 准确率', '94.1% (191/203)', 'AI推荐的第一个诊断编码命中正确编码的比例'],
...
add_para('手术操作编码因ICD-9-CM-3编码库体量庞大（超3,000条），目前优先覆盖高频编码571条（9个专科），F1得分暂为23.5%。我们选择公开这一数据，因为诚实比修饰更重要。手术编码不依赖LLM也可通过规则引擎提供基线结果——不是技术上做不到，而是编码库扩充的优先级排在诊断编码之后。改进计划：第1阶段（省赛前）手术编码库扩充至1,000条，F1目标提升至40%+；第2阶段（国赛前）扩充至1,500条，F1目标55%+。')
add_para('')
add_para('诊断编码准确率提升路线图：', bold=True)
add_para('当前94.1%的Top-1准确率已超过人工编码平均水平（85-90%），但仍有多条明确路径可以继续提升：')
add_para('')
add_table_simple(['阶段', '目标准确率', '实现路径', '需要资源'], [
    ['当前', '94.1%', '203份测试数据 + Qwen2.5-7B本地部署', '已有'],
    ['近期（省赛前）', '95-96%', '同义词库翻倍(每条编码从2-3个同义表述扩至5-8个)\n+ 测试数据从203份扩至500份', '标注人力：从病历中提取同义表述\n约需2-3周集中标注'],
...
add_para('')
add_para('核心逻辑：94.1%是起点不是终点。同义词库是近期最大杠杆（成本最低），LLM升级是中期最大杠杆（效果最显著），数据飞轮是长期壁垒（竞品最难复制）。', size=10, color=(0x94,0xA3,0xB8))
...
add_bullet('不是PPT创业：已有完整可运行的全栈系统（8页面+7组API+17个测试），203例基准测试诊断Top-1准确率94.1%，评审可现场验证任何功能。')
...
add_table_simple(['风险', '概率', '影响', '应对策略'], [
    ...
    ['编码覆盖面：920条仅覆盖\nICD-10总量的3-5%', '高(确定)', '中高',
     '优先覆盖前80%高频编码(覆盖95%+出院病例)；\n低频罕见病采用LLM语义兜底——大模型可推理\n不常见编码即使未经显式训练；\n建立编码库持续扩展机制(每月迭代)'],
...
add_table_simple(['数据项', '来源', '说明'], [
    ...
    ['编码准确率94.1%（Top-1）', '码医团队内部基准测试', '203份模拟临床病历，覆盖8个科室，测试环境与测试结果记录可提供'],
])
add_para('')
