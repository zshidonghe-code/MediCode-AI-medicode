...
add_table_simple(['项目', '内容'], [
    ...
    ['团队', '郑诗东和（项目负责人），利用AI辅助开发工具单人完成全栈开发'],
...
add_bullet('手术映射库：611条（持续扩充中）')
add_bullet('语义检索：TF-IDF字符级1-3 gram，5000特征维度')
add_bullet('当前验证数据（4例合成病历规则模式测试）：主诊断匹配2/4、手术编码匹配2/4，结果不代表临床准确率')
...
add_bullet('双模式演示：快速演示（比赛首选，规则引擎1秒出结果，离线安全）+ 完整演示（LLM增强+打字机动画）')
add_bullet('内置3个合成病历样本（心内科心肌梗死、呼吸科COPD急性加重、骨科股骨颈骨折）')
add_bullet('双引擎架构：规则引擎保底（离线可用）+ LLM大模型增强（联网更精准），核心功能不中断')
add_bullet('快速模式下顶栏显示"快速模式（离线）"标签，反而凸显架构健壮性')
add_bullet('自动触发全流程分析：编码→质控→DRG→费用4步，结果自动持久化到数据库')
add_bullet('完成后显示耗时，Dashboard实时反映最新数据')
add_bullet('速度可调：快（12ms/字）/ 中（28ms/字）/ 慢（55ms/字），完整模式下可用')
add_bullet('支持一键重播，可快速切换病例，文件上传支持 .txt / .docx / .pdf 三种格式')
...
add_bullet('ICD编码器：4层推荐策略，920条诊断映射和611条手术映射；当前4例规则模式验证结果见基准报告')
...
add_table_simple(['维度', '分值', '我们要证明的'], [
    ['创新性', '30分', 'AI辅助开发 + 自研医学知识库，不是简单调API。LLM + 向量检索 + 规则引擎三层架构，双引擎离线可用'],
    ['商业性', '30分', 'DRG付费改革是国家级刚需，医院编码科是真实付费方，ROI清晰可量化（>6:1）'],
    ['团队', '20分', '项目负责人利用AI辅助开发工具单人完成全栈开发（8页面+7组API+8张表），2个月完成传统团队3-6个月工作量，有完整可运行系统'],
...
add_table_simple(['时间段', '环节', '内容要点'], [
    ...
    ['4:00-5:00', '商业模式+愿景', '已开发完成可演示；准确率目标尚待扩大病例集和专家复核验证；让每一份病历都可追溯审核'],
])
...
add_table_simple(['角色', '成员', '职责'], [
    ['项目负责人', '郑诗东和', '方向决策、资源协调、商业计划书、路演PPT、路演演讲、医院合作洽谈。自学ICD编码规则和CHS-DRG分组方案，完成编码员深度访谈。'],
    ['技术开发', '郑诗东和（AI辅助）', '利用AI辅助开发工具（Claude），单人2个月内从零搭建完整全栈系统——8个前端页面、7组API服务、8张数据库表、17个单元测试。传统3-5人团队通常需要3-6个月。'],
    ['医学顾问', '招募中', '已与医学院和医院编码科取得联系，正在进行合作洽谈。同步通过大赛评委/校友/行业网络积极寻找。'],
])
...
add_table_simple(['时间', '里程碑'], [
    ...
    ['已完成', '4例规则模式基准测试：主诊断匹配2/4、手术编码匹配2/4，基于当前实现记录'],
